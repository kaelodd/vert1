...
const DecommissionedAssetsList = ({ list }) => {
    ...
    const viewAsset = (e, asset) => {
        ...
        if (asset) {
            ...
            modal.confirm({
                title: 'Decommissioned Asset Details',
                icon: <InfoCircleOutlined />,
                width: 800,
                content: (<>
                    <Content
                        className='decommissioned-asset-details'
                        style={{
                            margin: '24px 16px',
                            // padding: 12,
                            minHeight: 280,
                    ...
                    >
                        <Table style={{ fontSize: 'xx-small !important', tableLayout: 'fixed !important' }} columns={columns} dataSource={data} />
                    </Content>
                </>),
                onOk: () => {
                    setShowModal(false);
                ...
                renderItem={(item, index) => (
                    <List.Item className='assets-list-item' onClick={(e) => viewAsset(e, item)}>
                        <List.Item.Meta
                            ...
                            title={`${item?.assetCode || ''} ${item?.functionalLocation ? ' - ' + item?.functionalLocation : ''}`}
...
export default DecommissionedAssetsList;